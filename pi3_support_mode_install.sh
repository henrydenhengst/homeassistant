...
sudo apt install -y git curl wget python3-pip python3-venv mosquitto mosquitto-clients build-essential lirc ufw htop smartmontools unattended-upgrades
...
if sudo smartctl -a /dev/mmcblk0 | grep -q "Errors"; then
    echo "[$(date)] SD card reports errors, rebooting..." >> $LOG
...
mosquitto_pub -h "$MQTT_BROKER" -t test/pi -m "hello" && log "✅ MQTT publish OK" || { log "❌ MQTT publish FAILED"; ERRORS=$((ERRORS+1)); }
